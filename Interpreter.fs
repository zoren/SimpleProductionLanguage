...
module Interpreter =
    ...
    let bindRule facts (rule : Rule) : Map<LValue, RTValue> seq =
        ...
        binder Map.empty <| Set.toList lvalDom
    ...
    type Interpreter(rules : Rule seq) =
        ...
        member x.Add(fact:Fact) =
            let oldFacts = !facts
            if Set.contains fact oldFacts
            then failwith "Fact already added"            
            facts := leFix (Set.add fact oldFacts)

        member x.Remove(fact:Fact) = failwith ""
        
        member x.Modify(oldFact:Fact,newFact:Fact) = failwith ""
